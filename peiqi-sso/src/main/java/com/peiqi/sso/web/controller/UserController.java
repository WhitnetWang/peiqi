...
import com.peiqi.sso.service.RedisService;
import com.peiqi.sso.service.UserService;
...
public class UserController {
	...
	public void login(User user, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(u == null) {
			...
			request.setAttribute("message", "您输入的账号有误");
			request.getRequestDispatcher("/WEB-INF/views/msg.jsp").forward(request, response);
			
		}else {
			
			String key = "TOKEN_" + u.getUsername() + System.currentTimeMillis();
			
			String value = JSONObject.fromObject(u).toString();
			
			redisService.set(key, value, 60 * 30);
			
			Cookie cookie = new Cookie("PEIQI_USER_TOKEN", key);
			cookie.setPath("http://www.peiqi.com");
			cookie.setMaxAge(60 * 60);
			cookie.setDomain("peiqi.com");
			
			response.addCookie(cookie);
			response.sendRedirect("http://www.peiqi.com");
			return ;
		}
	...
	@RequestMapping("/checkcodeImgAction.action")
	public void checkcodeImg(HttpServletRequest request,HttpServletResponse response) throws Exception {
		int width = 120;
		int height = 30;
	...
	@RequestMapping("queryUserByToken.action")
	public void queryUserByToken(String key,HttpServletResponse response) throws IOException {
